...
function getManagement() {
	...
	const managementPort = serverRuntimeConfig[Symbol.for("__NEXT_WORKER_MANAGEMENT_PORT__")];
	const management = sharedObjects.management = respawnComlinkProxy(() => {
		const instance = newClient(managementPort, "management", "management");
		instance.context.changeCallPayloadStrategy("childProcess");
		instance.context.socket.addListener("close", () => delete sharedObjects.management);
		return instance;
	});

	log(`Updating latest next config to management`);
	const managementExports = management.exports;
	managementExports[Comlink.syncSymbol].updateNextConfig({
		serverRuntimeConfig: serverRuntimeConfig.toJSON(),
		publicRuntimeConfig: publicRuntimeConfig.toJSON()
	});
	return management;
}
function getManagementMemory() {
	if(sharedObjects.managementMemory != null)
		return sharedObjects.managementMemory;
	const { serverRuntimeConfig } = getConfig();
	const managementMemoryId = serverRuntimeConfig[Symbol.for("__NEXT_WORKER_MANAGEMENT_MEMORY_ID__")];
	const managementMemory = new sharedMemory(managementMemoryId, 8192, false, false);
	return sharedObjects.managementMemory = managementMemory;
}
function readManagementMemoryCache() {
	const managementMemory = getManagementMemory();
	const buffer = managementMemory.readBuffer();
	const length = buffer.readUInt32BE(0);
	if(length == 0) return {};
	const bufferPayload = buffer.subarray(4, length + 4);
	let desia = sharedObjects.desia;
	if(desia == null)
		desia = sharedObjects.desia = new DeSia({ refSize: 16 });
	const entries = desia.deserialize(bufferPayload);
	return Object.fromEntries(entries
		.map(([id, hash, port]) => ({ id, hash, port }))
		.map(v => [`worker-cache-${v.id}-${v.hash}`, v]));
}
...
function initSharedObjects({ syncWorkerPath }) {
	...
	if(sharedObjects.log == null)
		sharedObjects.log = debug("build:plugin:withWorker:ipc-server:parent");
	if(sharedObjects.logger == null)
		sharedObjects.logger = newLogger("worker-parent");
}

function newInstance(path, id, hash) {
	...
	workerInstance.client = respawnComlinkProxy(() => {
		if(workerInstance.spawnCount++ > 0)
			log(`Respawning worker ${id}-${hash}`);
		const workerCache2 = readManagementMemoryCache()[cacheName];
		if(workerCache2 != null && workerCache2.port != null) {
			log(`Using fasttrack cache for worker ${id}-${hash}`);
			const instance = newClient(workerCache2.port, id, hash);
			instance.context.socket.addListener("close", () => delete sharedObjects[cacheName]);
			return instance;
		}
		// Don't worry about synchronous call stalling. The strategy has already been
		// set to use child process instead.
		log(`Getting worker ${id}-${hash} from management`);
		const managementExports = getManagement().exports;
		const worker = managementExports[Comlink.syncSymbol].spawnWorker(path, id, hash);
		const instance = newClient(worker.port, id, hash);
		instance.context.socket.addListener("close", () => delete sharedObjects[cacheName]);
		return instance;
	});
...
function newClient(workerPort, workerId, workerHash) {
	...
	const ipcContext = (() => {
		const context = {};
		const socket = context.socket = net.connect({ port: workerPort }, () => context.ipcId = socket.localPort);
		context.ipcId = socket.localPort;
