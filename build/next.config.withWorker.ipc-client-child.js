...
const {
	...
	isInSharedWorker,
	isInServiceWorker,
	getGlobalDedicatedWorkerAsPort,
	getGlobalSharedWorkerAsPort,
	getGlobalIframeWorkerAsPort
...
function initSharedObjects({}) {
	if(!sharedObjects.childInitialized) {
		...
		sharedObjects.attachExceptionHook = () => {
			globalThis.addEventListener("uncaughtException", event => {
...
function newInstanceManagement() {
	const id = "management";
	const hash = "management";
	sharedObjects.log = debug(`build:plugin:withWorker:ipc-client:management`);
	sharedObjects.logger = newLogger("worker-management");
	log(`Initializing management`);
	const port = getGlobalIframeWorkerAsPort();
	const server = newServer(port, id, hash);
	const oldExpose = server.expose.bind(server);
	server.expose = async object => {
		await oldExpose(object);
		sharedObjects.attachExceptionHook();
	};
	return server;
}
...
function newInstance() {
	const workerDataPayload = Buffer.from(location.hash.slice(1), "base64");
	const workerData = new DeSia().deserialize(workerDataPayload);
	const id = workerData.id;
	const hash = workerData.hash;
	sharedObjects.log = debug(`build:plugin:withWorker:ipc-client:child:${id}-${hash}`);
	sharedObjects.logger = newLogger(`worker-${id}-${hash}`);
	log(`Initializing worker`);
	setConfig(workerData.initialNextConfig);
	let port;
	if(isInDedicatedWorker())
		port = getGlobalDedicatedWorkerAsPort();
	else if(isInSharedWorker())
		port = getGlobalSharedWorkerAsPort();
	// else if(isInServiceWorker())
	// 	port = getGlobalServiceWorkerAsPort();
	else
		throw new Error("Invalid worker environment");
	const server = newServer(port, id, hash);
	let resolveManagementPort;
	let rejectManagementPort;
	sharedObjects.managementPort = new Promise((resolve, reject) => {
		resolveManagementPort = resolve;
		rejectManagementPort = reject;
	});
	port.addEventListener("message", event => {
		...
		switch(command) {
			...
			case "setManagementPort": {
				log(`Setting management port`);
				if(value != null) {
					const [managementPort, ipcId] = value;
					managementPort.ipcId = ipcId;
					resolveManagementPort(managementPort);
				} else
					rejectManagementPort(new Error("Management port is not available"));
				if(exposed)
					(event.port || port).postMessage({ command: "ready", value: null });
				break;
			}
		}
	});
	...
	server.expose = async object => {
		...
		exposed = true;
		sharedObjects.attachExceptionHook();
		port.postMessage({ command: "ready", value: null });
	};
	return server;
}
...
function newServer(workerPort, workerId, workerHash) {
	...
	const newClient = (/** @type MessagePort */ socket) => {
		const context = {};
		context.socket = socket;
		context.ipcId = socket.ipcId;
		...
		const xhrCallSync = buffer => {
			...
			xhr.setRequestHeader("X-IPC-Side", "child");
		...
		(async () => {
			...
			const serviceSocket = context.serviceSocket = new BroadcastChannel(`${workerId}-${workerHash}-${context.ipcId}-child`);
	...
	workerPort.addEventListener("message", event => {
		...
		if(command != "newPort") return;
		const { port1, port2 } = new MessageChannel();
		port2.ipcId = value;
		newClient(port2);
		(event.port || workerPort).postMessage({ id: id, value: port1 }, [port1]);
	});
